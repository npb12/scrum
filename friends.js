	...
	xmlhttp.onreadystatechange = function() 
	{
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
			{ 
				...
				if (friends_list.length> 0)
				{
					var friendshtml = "<p></p><h2>Associates</h2><table id='friendsPageTable' >";
					for(var i = 0; i < friends_list.length; i += 1)
					{
						var userid = friends_list[i].getElementsByTagName('userid')[0].textContent;
						var fname = friends_list[i].getElementsByTagName('fname')[0].textContent;
						...
						switch(friendsstatus)
						{
							case -1:
								...
								break;
							case 0:
								friendshtml += "<button type=\"button\" style=\"font: 24px;\" onclick=\"window.location = 'addfriend.php?id=" + userid + "';\">+1 add</button>";
								break;
							case 1:
								friendshtml += "<img src='check.png' width='12' height='13' />Associates";
								break;
							case 2:
								friendshtml += "Friend request pending!";
								break;
				...
				{
					document.getElementById('friendslist').innerHTML = "<h3>You have no Associates:(</h3>";
				} 
			}
